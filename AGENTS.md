...
FalconShot：Windows 智能截图工具（截图 → 标注 → 贴图 / OCR / AI 识别）。Rust workspace + Tauri 2 + React 19。目标平台 Windows 10/11；UI 支持简体中文/英文，默认跟随系统语言（`general.language` 默认 `"system"`）。
...
- **settings**：`settings-core/types.rs` 是唯一 schema；新增字段必须 `#[serde(default)]`（兼容旧 settings.json）。保存后 `save_settings` 会重新应用全局热键，并在界面语言变化时重建托盘菜单。
- **多语言（i18n）**：前端用 react-i18next，**禁止在组件里写中文字面量**——新文案必须同时在 `src/locales/zh-CN.json` 和 `en.json` 加同名键；语言解析在 `src/i18n.ts`（设置值 `"system"`/`"zh-CN"`/`"en"`，system 跟随 `navigator.language`）。Rust 侧用户可见文案（托盘、窗口标题、错误信息）统一走 `src-tauri/src/i18n.rs` 的 `tr()`；新增 Rust 字符串时两种 Lang 都要补。AI 提示词（`ai.rs` 的 DEFAULT_*_PROMPT）是内容不是 UI，保持中文不翻译。
- **前端**：React 19 + Tailwind；`react-markdown` 用于渲染 AI 输出。画布标注导出用 `canvas.toBlob`（CSS 边框/光晕不会进入导出图）。
